 ...
 *
 * This is used to "negotiate" connection types between two separate
 * IEnergyTransports, allowing users to set flow direction and allowing for
...
public interface IEnergyTransport extends IEnergyProvider, IEnergyReceiver {
	...
	enum InterfaceType {
		/**
		 * Indicates that this {@link IEnergyTransport} is only sending power on
		 * this side.
		 */
		SEND,
		/**
		 * Indicates that this {@link IEnergyTransport} is only receiving power
		 * on this side.
		 */
		RECEIVE,
		/**
		 * Indicates that this {@link IEnergyTransport} wants to balance power
		 * between itself and the senders/receivers on this side. This is the
		 * default state.<br>
		 * To block any connection, use
		 * {@link IEnergyConnection#canConnectEnergy}
		 *
		 * IEnergyTransport based senders should check that the total power in
		 * the destination IEnergyTransport is less than the power in themselves
		 * before sending. <br>
		 * Active IEnergyTransport receivers (i.e., those that call
		 * {@link IEnergyProvider#extractEnergy}) should check that they contain
		 * less power than the source IEnergyTransport.
		 */
		BALANCE;
		 ...
		 * 
		 * @return The opposite {@link InterfaceType}
		 */
		 ...
		 *
		 * @param forward
		 *            Whether to step in the order specified by
		 *            {@link IEnergyTransport#getTransportState} (<tt>true</tt>)
		 *            or to step in the opposite direction
		 * @return The new {@link InterfaceType} when rotated
		 */
	...
	/**
	 * Indicates to other IEnergyTransports the state of the given side. See
	 * {@link InterfaceType} for details.
	 *
	 * For clarity of state tracking, on a tile update from another
	 * IEnergyTransport, if its mode has changed from the opposite of your own
	 * mode on that side, you should change your mode to the opposite of its
	 * mode.
	 *
	 ...
	 *            The side
	 *
	 * @return The type of connection to establish on this side. <b>null is NOT
	 *         a valid value</b>
	 */
	InterfaceType getTransportState(EnumFacing from);

	/**
	 * This method is provided primarily for the purposes of automation tools,
	 * and should not need to be called by another IEnergyTransport.
	 *
	 * Calls to this method may fail if this IEnergyTransport has been secured
	 * by a user.
	 *
	 * @param state
	 *            The new state
